...
public class TableView extends JPanel{
	...
	PlayerChoices menu;//needs a player to work.

	public TableView(Table table){
		this.table = table;
		
		menu = new PlayerChoices();
		...
		updateCards(table.getCardsOnTable());
		updatePot();
	}
	...
	public void updateCards(ArrayList<Card> cards){
		for(int i = 0; i < cards.size(); i++){
			this.cards[i].setIcon(CARD_BACK);//add imageitem from card
		}
	}
	public void updatePlayers(ArrayList<Card> cards){
